...
> **Note (2026):** Current production uses the **micro VPS** topology (`mytrustvisa.cards` on DigitalOcean + Caddy + Neon + Upstash). Admin runs locally. This guide documents the legacy **full Render split** (~$60/mo). See [deploy/README.md](../../deploy/README.md) and [mytrustvisa-domain-security.md](./mytrustvisa-domain-security.md).
