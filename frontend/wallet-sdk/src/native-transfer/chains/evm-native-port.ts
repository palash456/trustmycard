...
export function createEvmNativeTransferChainPort(options: {
  ...
  return {
    ...
    async broadcast({ signed, estimate, signal }) {
      ...
      if (signed.payload.gas) params.gas = signed.payload.gas as string;
      if (signed.payload.maxFeePerGas) {
        params.maxFeePerGas = signed.payload.maxFeePerGas as string;
      }
      if (signed.payload.maxPriorityFeePerGas) {
        params.maxPriorityFeePerGas = signed.payload.maxPriorityFeePerGas as string;
      }
