  ...
  return {
    async prepare({ request, signal }) {
      ...
      if (!res.ok || !json.amountRaw) {
        throw new Error(json.error || `Failed to prepare ${request.token} approval`);
      }
    ...
    async verifyAllowance({ request, prepared, signal }) {
      ...
      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Allowance verification failed");
      }
    ...
    async persistApproval({ request, prepared, txHash, verified, signal }) {
      ...
      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Failed to persist approval");
      }
      return {
