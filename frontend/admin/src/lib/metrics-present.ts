...
export function normalizeMetricsSnapshot(raw: unknown): NormalizedMetrics | null {
  ...
  if (Array.isArray(data.counters)) {
    ...
    return {
      capturedAt: String(data.ts ?? data.capturedAt ?? new Date().toISOString()),
      counters,
      histograms: Array.isArray(data.histograms)
        ? (data.histograms as HistogramSnapshot[])
        : [],
      gauges: Array.isArray(data.gauges) ? (data.gauges as GaugeSnapshot[]) : [],
    };
  }

  if (data.counters && typeof data.counters === "object") {
    const counters = Object.entries(data.counters as Record<string, number>).map(
      ([name, value]) => ({
        name,
        labels: {},
...
export function formatMetricLabels(labels: Record<string, string | number | boolean>): string {
