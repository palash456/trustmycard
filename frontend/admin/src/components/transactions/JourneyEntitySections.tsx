...
export function JourneyEntitySections({
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <EntityTable title={`Approvals (${data.approvals.length})`} empty="No approvals linked to this transaction" hasItems={data.approvals.length > 0}>
        <ul className="space-y-2 text-sm">
            {data.approvals.map((a) => (
              <li key={a.id} className="flex flex-wrap items-center gap-2 rounded-md border px-3 py-2">
                <StatusBadge value={a.status} />
                <Link href={`/approvals/${entityRouteId(a)}`} className="font-medium text-primary hover:underline">
        ...
        <ul className="space-y-2 text-sm">
          {data.transfers.map((t) => (
            <li key={t.id} className="flex flex-wrap items-center gap-2 rounded-md border px-3 py-2">
              ...
              <Link href={`/transfers/${entityRouteId(t)}`} className="font-medium text-primary hover:underline">
            ...
            {data.tgEvents.map((e) => (
              <li key={e.id} className="rounded-md border px-3 py-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">{e.network.toUpperCase()}</span>
      ...
      >
        <ul className="space-y-2 text-sm">
            {data.nativeTransfers.map((n) => (
              <li key={n.id} className="flex flex-wrap items-center gap-2 rounded-md border px-3 py-2">
