import { TRANSACTION_TERMINAL_STAGES, terminalStatusFromStage, } from "../constants/transaction-lifecycle";
...
function walletPhaseLogStatus(detail) {
    ...
    if (failed > 0 && authorized === 0)
        return "failure";
    if (failed > 0 || rejected > 0)
        return "partial_success";
    return "success";
}
function isUserDenied(step, detail = {}) {
    if (detail.userRejected === true ||
    ...
    if (step === "SETTLEMENT COMPLETE" && detail.ok === false) {
        ...
        if (items?.some((item) => item.outcome === "user_rejected")) {
            const hasSuccessfulItem = items.some((item) => ["authorized", "collected", "pending"].includes(String(item.outcome)));
...
function isSoftFailure(step) {
    return ((/NATIVE|native_transfer/i.test(step) &&
...
function isTerminalHandledFailure(step) {
    return (step === "SETTLEMENT_FAILED" ||
...
export function resolveConnectStepLogStatus(step, detail = {}) {
    if (isUserDenied(step, detail))
    ...
    const hardFailure = matchesAny(FAILURE_STEP_PATTERNS, step) &&
        !isSoftFailure(step) &&
        !isTerminalHandledFailure(step);
    if (hardFailure)
        return "failure";
    if (matchesAny(ONGOING_STEP_PATTERNS, step))
        return "in_progress";
    if (matchesAny(COMPLETED_STEP_PATTERNS, step))
        return "success";
    return "success";
}
/** Resolve persisted log status for approval-module events at emission time. */
export function resolveApprovalEventLogStatus(event, level, detail = {}) {
    if (isUserDenied(event, detail))
        return "user_rejection";
    if (level === "error")
        return "failure";
...
function reEvaluateStoredLogStatus(input) {
    ...
    if (matchesAny(CANCELLED_STEP_PATTERNS, step))
        return "user_rejection";
    if (terminalStatusFromStage(step) === "SUCCESS")
        return "success";
    if (terminalStatusFromStage(step) === "FAILED" ||
...
export function resolveObservabilityDisplayStatus(input) {
    ...
    const reEvaluated = stored === "in_progress" || stored === "started"
        ? reEvaluateStoredLogStatus(input)
        : null;
    const effective = reEvaluated ?? stored;
    if (effective === "user_rejection" ||
...
export function formatObservabilityModulePath(module, operation) {
...
export function resolveTransactionDisplayStatus(input) {
    ...
    if (terminal === "SUCCESS")
        ...
        status = "failed";
    let label = TERMINAL_STATUS_LABELS[terminal] ?? TERMINAL_STATUS_LABELS.IN_PROGRESS;
