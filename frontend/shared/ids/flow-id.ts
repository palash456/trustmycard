...
const SEMANTIC_FLOW_ID =
  /^flow-(\d{8})-(\d{6})-([A-Z0-9]{6})(?:-([A-Z0-9]{2}))?$/;
...
export function generateFlowId(input: GenerateFlowIdInput): string {
...
export function isSemanticFlowId(id: string): boolean {
  return SEMANTIC_FLOW_ID.test(id.trim());
}

/** Pre-semantic opaque client IDs (still valid for lookup). */
export function isLegacyFlowId(id: string): boolean {
  const trimmed = id.trim();
  if (!trimmed.startsWith("flow-")) return false;
  if (isSemanticFlowId(trimmed)) return false;
...
export function journeyCoreFromFlowId(flowId: string): string | null {
...
export function parseSemanticFlowId(flowId: string): {
  ymd: string;
  hms: string;
  walletSuffix: string;
  collisionSuffix?: string;
