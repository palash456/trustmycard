  ...
  onModuleInit(): void {
    ...
    this.worker = new Worker(
      ...
      async (job) => {
        const delivery = await this.prisma.merchantWebhookDelivery.findFirst({
          where: {
            ...
            collectionIntentId: job.data.intentId,
          },
        });
        ...
        const secret =
          this.platformConfig.getMonitoring().merchantWebhookSecret;
        const response = await fetch(delivery.endpoint, {
          ...
          signal: AbortSignal.timeout(
            this.platformConfig.getMonitoring().merchantWebhookTimeoutMs,
          ),
        });
        if (!response.ok)
      ...
      },
      {
        ...
        concurrency:
          this.platformConfig.getCollection().merchantWebhookConcurrency,
      },
    );
  }
